...
> 实现状态说明：本目录同时包含早期产品目标和长期架构设计，不应全部视为当前已实现能力。当前 Godot 客户端使用账号登录与游戏 Session 交换，不自动创建匿名账号；真实启动、离线边界、同步状态和测试命令以仓库根 `README.md` 与 `client/README.md` 为准。
...
- 原始 P0 方案采用匿名登录；当前实现已切换为账号登录和 Session 交换。匿名会话 API 仍由服务端保留，但不在当前客户端启动流程中使用。
