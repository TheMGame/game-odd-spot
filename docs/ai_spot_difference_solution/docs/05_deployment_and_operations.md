...
目标设计是启动先使用本地配置快照，再异步请求 bootstrap，并预加载后续关卡。当前实现会在有效 Session 下请求 bootstrap 后进入首页；Catalog 使用内存/磁盘缓存，图片缓存上限为 300MB 并校验 sha256。关卡 JSON 完整离线缓存及“离线可玩已缓存关卡”仍未全部实现，首次安装必须联网。
