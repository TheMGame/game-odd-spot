...
const state={catalog:[],home:{},museum:{title:"大侦探博物馆",subtitle:"",items:[]},assets:[],view:"overview",series:null,level:null,levelEntry:null,selected:0,dirty:false,adding:false,puzzleSelected:-1,puzzleView:"mixed"};
...
function renderStoryEditor(){const l=state.level,story=l.story||(l.story=defaultStory()),nodes=Object.entries(story.nodes||{});$("view").innerHTML=`<div class="editor"><aside class="editor-left"><div class="editor-head"><h2>${escapeHtml(l.title)}</h2><small>${escapeHtml(l.level_id)} · v${l.level_version}</small></div><div class="panel-section"><div class="panel-title">故事信息</div><div class="form-stack"><label>标题<input value="${escapeHtml(l.title)}" oninput="updateLevelField('title',this.value)"></label><label>玩法<select onchange="changeLevelMode(this.value)"><option value="find_anachronism">找穿帮</option><option value="image_puzzle">拼图（错位还原）</option><option value="interactive_story" selected>互动故事</option></select></label><label>所属系列<select id="editor-series" onchange="markDirty()">${state.catalog.map(s=>`<option value="${s.id}" ${s.id===state.series?.id?"selected":""}>${escapeHtml(s.title)}</option>`).join("")}</select></label><label>难度<select onchange="updateDifficulty(this.value)">${[1,2,3,4,5].map(n=>`<option ${n===(l.difficulty?.total||1)?"selected":""}>${n}</option>`).join("")}</select></label></div></div><div class="panel-section"><div class="panel-title">节点 ${nodes.length}</div>${nodes.map(([id,n])=>`<div class="answer-item"><span class="answer-number">${escapeHtml(n.type.slice(0,1).toUpperCase())}</span><div><b>${escapeHtml(n.title||n.speaker||id)}</b><br><small>${escapeHtml(id)} · ${escapeHtml(n.type)}</small></div></div>`).join("")}</div></aside><section class="canvas-pane"><div class="canvas-toolbar"><b>故事 JSON</b><span>起点：${escapeHtml(story.start_node||"")}</span></div><div style="padding:20px;height:calc(100vh - 140px);background:#f5f6f8"><textarea spellcheck="false" style="width:100%;height:calc(100% - 35px);font:13px/1.55 Consolas,monospace;resize:none" oninput="storyJsonChanged(this.value)">${escapeHtml(JSON.stringify(story,null,2))}</textarea><small id="story-json-error">支持 scene、dialogue、choice、hotspot、evidence、sequence、puzzle、ending；图片和音频填写素材库返回的完整资源描述。</small></div><footer class="editor-footer"><span id="editor-status">修改后请保存</span><div class="actions"><button class="btn" onclick="saveLevel('draft')">保存草稿</button><button class="btn primary" onclick="saveLevel('published')">发布故事</button></div></footer></section><aside class="inspector"><div class="panel-section"><h2>跨端故事</h2><p>Web 与微信小游戏读取同一份节点数据。请勿粘贴 HTML 或脚本；每个图片、背景音乐、配音都使用素材库中的 HTTPS 资源描述。</p><p><b>互动约定</b><br>choice 使用 choices；hotspot 使用 0～1 坐标；sequence 配置 items 与 correct_order；ending 至少一个。</p></div></aside></div>`}
