...
const state={catalog:[],museum:{title:"大侦探博物馆",subtitle:"",items:[]},assets:[],view:"overview",series:null,level:null,levelEntry:null,selected:0,dirty:false,adding:false,puzzleSelected:-1,puzzleView:"mixed"};
...
async function loadData(){const [catalog,assets]=await Promise.all([api("/catalog"),api("/assets")]);state.catalog=catalog.series||[];state.museum=catalog.museum||{title:"大侦探博物馆",subtitle:"",items:[]};state.assets=assets.items||[]}
...
function navigate(view){if(state.dirty&&!confirm("当前关卡有未保存修改，确定离开吗？"))return;({overview:renderOverview,series:renderSeries,levels:renderLevels,library:renderLibrary,museum:renderMuseum,review:()=>renderReview(),feedback:()=>renderPlaceholder("feedback","问题反馈","集中处理玩家上报的问题"),analytics:()=>renderPlaceholder("analytics","数据分析","查看完成率、提示使用与热点误触"),settings:()=>renderSettings()})[view]()}

function museumItemRow(item,index){const assets=state.assets.filter(a=>a.kind==="image");return `<section class="card" style="margin-bottom:14px"><div class="page-header" style="margin-bottom:12px"><div><h3 style="margin:0">${escapeHtml(item.name||"新藏品")}</h3><small>${escapeHtml(item.id||"")}</small></div><div class="actions"><button class="btn small" onclick="moveMuseumItem(${index},-1)">↑</button><button class="btn small" onclick="moveMuseumItem(${index},1)">↓</button><button class="btn small danger" onclick="removeMuseumItem(${index})">删除</button></div></div><div class="form-stack"><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><label>藏品 ID<input value="${escapeHtml(item.id||"")}" oninput="updateMuseumItem(${index},'id',this.value)"></label><label>名称<input value="${escapeHtml(item.name||"")}" oninput="updateMuseumItem(${index},'name',this.value)"></label></div><label>说明<textarea oninput="updateMuseumItem(${index},'description',this.value)">${escapeHtml(item.description||"")}</textarea></label><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><label>分组<input value="${escapeHtml(item.group||"")}" oninput="updateMuseumItem(${index},'group',this.value)"></label><label>排序<input type="number" value="${Number(item.sort_order)||0}" oninput="updateMuseumItem(${index},'sort_order',Number(this.value))"></label></div><label>藏品图片<select onchange="updateMuseumItem(${index},'image_url',this.value)"><option value="${escapeHtml(item.image_url||"")}">${escapeHtml(item.image_url||"请选择素材")}</option>${assets.map(a=>`<option value="${escapeHtml(a.url)}">${escapeHtml(a.name||a.asset_id)}</option>`).join("")}</select></label><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><label>解锁条件<select onchange="updateMuseumItem(${index},'unlock_type',this.value)"><option value="always" ${item.unlock_type==="always"?"selected":""}>直接解锁</option><option value="level_complete" ${item.unlock_type==="level_complete"?"selected":""}>完成指定关卡</option><option value="series_complete" ${item.unlock_type==="series_complete"?"selected":""}>完成指定系列</option></select></label><label>关卡/系列 ID<input value="${escapeHtml(item.unlock_value||"")}" oninput="updateMuseumItem(${index},'unlock_value',this.value)" placeholder="解锁条件对应 ID"></label></div><label><input type="checkbox" ${item.enabled!==false?"checked":""} onchange="updateMuseumItem(${index},'enabled',this.checked)"> 启用藏品</label></div></section>`}
