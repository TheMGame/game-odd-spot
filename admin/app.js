...
function levelTable(levels,series){
  ...
  return `<table class="level-table"><thead><tr><th>预览</th><th>关卡</th><th>玩法</th><th>内容数</th><th>难度</th><th>版本</th><th></th></tr></thead><tbody>${levels.map(l=>`<tr><td><img class="thumb" src="${escapeHtml(l.thumbnail_url)}"></td><td><b>${escapeHtml(l.title)}</b><br><small>${escapeHtml(l.id)}</small></td><td>${l.mode==="image_puzzle"?"拼图":"找穿帮"}</td><td>${l.content_count??l.difference_count??0}</td><td>${"◆".repeat(l.difficulty||1)}</td><td>v${l.version}</td><td><button class="btn small" onclick="openEditorById('${l.id}','${series?.id||findSeries(l.id)?.id||""}')">编辑</button></td></tr>`).join("")}</tbody></table>`;
}
...
function renderPuzzleEditor(){
  const l=state.level,p=l.puzzle||(l.puzzle={rows:5,cols:4,operations:[]}),asset=l.assets?.image||{};
  const ops=p.operations||[];
  $("view").innerHTML=`<div class="editor"><aside class="editor-left"><div class="editor-head"><h2>${escapeHtml(l.title)}</h2><small>${escapeHtml(l.level_id)} · v${l.level_version}</small></div><div class="panel-section"><div class="panel-title">关卡信息</div><div class="form-stack"><label>标题<input value="${escapeHtml(l.title)}" oninput="updateLevelField('title',this.value)"></label><label>玩法<select onchange="changeLevelMode(this.value)"><option value="find_anachronism">找穿帮</option><option value="image_puzzle" selected>拼图（错位还原）</option></select></label><label>所属系列<select id="editor-series" onchange="markDirty()">${state.catalog.map(s=>`<option value="${s.id}" ${s.id===state.series?.id?"selected":""}>${escapeHtml(s.title)}</option>`).join("")}</select></label><label>难度<select id="level-difficulty" onchange="updateDifficulty(this.value)">${[1,2,3,4,5].map(n=>`<option ${n===(l.difficulty?.total||1)?"selected":""}>${n}</option>`).join("")}</select></label></div></div><div class="panel-section"><div class="panel-title">拼图设置</div><div class="coord-grid"><label>行<input type="number" min="2" max="8" value="${p.rows}" onchange="updatePuzzleGrid('rows',this.value)"></label><label>列<input type="number" min="2" max="8" value="${p.cols}" onchange="updatePuzzleGrid('cols',this.value)"></label><label>错位块<input id="puzzle-random-count" type="number" min="2" max="24" step="2" value="${Math.max(4,ops.length*2)}"></label></div><p>已配置错位 ${ops.length*2} 块</p><div class="answer-list">${ops.map((op,i)=>`<div class="answer-item"><span class="answer-number">${i+1}</span><b>${String(op.cells[0]+1).padStart(2,"0")} ↔ ${String(op.cells[1]+1).padStart(2,"0")}</b><button class="btn small danger" onclick="removePuzzleOperation(${i})">删除</button></div>`).join("")||'<small>点击图片上的两个格子添加交换</small>'}</div><div class="actions" style="margin-top:10px"><button class="btn small" onclick="randomizePuzzle()">随机生成</button><button class="btn small" onclick="resetPuzzle()">重置错位</button></div></div></aside><section class="canvas-pane"><div class="canvas-toolbar"><div class="canvas-tools"><button class="btn small ${state.puzzleView==="original"?"primary":""}" onclick="state.puzzleView='original';drawPuzzleAdmin()">原图</button><button class="btn small ${state.puzzleView!=="original"?"primary":""}" onclick="state.puzzleView='mixed';drawPuzzleAdmin()">错位效果</button></div><span>${l.assets?.width||0} × ${l.assets?.height||0}</span></div><div class="canvas-workspace"><canvas id="puzzle-admin-canvas" class="puzzle-admin-canvas" onclick="puzzleAdminClick(event)" data-src="${escapeHtml(asset.url||asset.local_path||"")}"></canvas></div><footer class="editor-footer"><span id="editor-status">点击两个格子配置交换</span><div class="actions"><button class="btn" onclick="showMobilePreview()">手机预览</button><button class="btn" onclick="saveLevel('draft')">保存草稿</button><button class="btn primary" onclick="showPublishModal()">发布关卡</button></div></footer></section><aside class="inspector" id="inspector"><div class="panel-section"><h2>拼图说明</h2><p>${state.puzzleSelected<0?"点击两个格子即可配置一组交换。一个格子只能参与一组初始交换。":`已选择 ${String(state.puzzleSelected+1).padStart(2,"0")}，请选择第二个格子。`}</p>${state.puzzleSelected>=0?'<button class="btn" onclick="state.puzzleSelected=-1;renderEditor()">取消选择</button>':''}</div></aside></div>`;
  drawPuzzleAdmin();
}
function changeLevelMode(mode){if(mode===state.level.mode)return;if(!confirm("切换玩法会重置当前玩法配置，是否继续？")){renderEditor();return}if(mode==="image_puzzle"){delete state.level.differences;state.level.puzzle={rows:5,cols:4,operations:[]};state.level.instruction="交换错位区域，还原完整画面"}else{delete state.level.puzzle;state.level.differences=[];state.level.instruction="圈出不属于这个年代的物件"}state.level.mode=mode;state.puzzleSelected=-1;markDirty();renderEditor()}
function puzzleOrder(){const p=state.level.puzzle,n=p.rows*p.cols,order=Array.from({length:n},(_,i)=>i);for(const op of p.operations||[]){const[a,b]=op.cells;[order[a],order[b]]=[order[b],order[a]]}return order}
function drawPuzzleAdmin(){const canvas=$("puzzle-admin-canvas");if(!canvas)return;const img=new Image();img.crossOrigin="anonymous";img.onload=()=>{const p=state.level.puzzle,maxW=760,maxH=Math.max(400,innerHeight-180),scale=Math.min(maxW/img.naturalWidth,maxH/img.naturalHeight,1);canvas.width=Math.round(img.naturalWidth*scale);canvas.height=Math.round(img.naturalHeight*scale);const ctx=canvas.getContext("2d"),order=state.puzzleView==="original"?Array.from({length:p.rows*p.cols},(_,i)=>i):puzzleOrder();for(let cell=0;cell<order.length;cell++){const piece=order[cell],dc=cell%p.cols,dr=Math.floor(cell/p.cols),sc=piece%p.cols,sr=Math.floor(piece/p.cols),dw=canvas.width/p.cols,dh=canvas.height/p.rows;ctx.drawImage(img,sc*img.naturalWidth/p.cols,sr*img.naturalHeight/p.rows,img.naturalWidth/p.cols,img.naturalHeight/p.rows,dc*dw,dr*dh,dw,dh);ctx.strokeStyle="#ffffffaa";ctx.strokeRect(dc*dw,dr*dh,dw,dh);ctx.fillStyle="#111c";ctx.font="bold 13px sans-serif";ctx.fillText(String(cell+1).padStart(2,"0"),dc*dw+7,dr*dh+18)}if(state.puzzleSelected>=0){const c=state.puzzleSelected%p.cols,r=Math.floor(state.puzzleSelected/p.cols);ctx.strokeStyle="#ffd54f";ctx.lineWidth=4;ctx.strokeRect(c*canvas.width/p.cols,r*canvas.height/p.rows,canvas.width/p.cols,canvas.height/p.rows)}};img.src=canvas.dataset.src}
function puzzleAdminClick(e){if(state.puzzleView==="original")return;const p=state.level.puzzle,rect=e.currentTarget.getBoundingClientRect(),col=Math.min(p.cols-1,Math.floor((e.clientX-rect.left)/rect.width*p.cols)),row=Math.min(p.rows-1,Math.floor((e.clientY-rect.top)/rect.height*p.rows)),cell=row*p.cols+col,used=new Set((p.operations||[]).flatMap(op=>op.cells));if(used.has(cell)){toast(`格子 ${String(cell+1).padStart(2,"0")} 已参与另一组交换，请先删除原交换。`,"error");return}if(state.puzzleSelected<0){state.puzzleSelected=cell;renderEditor();return}if(state.puzzleSelected===cell){state.puzzleSelected=-1;renderEditor();return}p.operations.push({type:"swap",cells:[state.puzzleSelected,cell]});state.puzzleSelected=-1;markDirty();renderEditor()}
function removePuzzleOperation(i){state.level.puzzle.operations.splice(i,1);markDirty();renderEditor()}
function resetPuzzle(){state.level.puzzle.operations=[];state.puzzleSelected=-1;markDirty();renderEditor()}
function updatePuzzleGrid(key,value){const p=state.level.puzzle,next=Math.max(2,Math.min(8,Number(value)||2));p[key]=next;if(p.rows*p.cols>48){p[key]=key==="rows"?Math.floor(48/p.cols):Math.floor(48/p.rows);toast("网格最多 48 格","error")}p.operations=[];state.puzzleSelected=-1;markDirty();renderEditor()}
function randomizePuzzle(){const p=state.level.puzzle,n=p.rows*p.cols;if(n%2){toast("全错位配置要求网格总数为偶数，请调整行或列","error");return}let permutation;for(let attempt=0;attempt<40;attempt++){const cells=Array.from({length:n},(_,i)=>i);for(let i=cells.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[cells[i],cells[j]]=[cells[j],cells[i]]}const next=Array(n).fill(0);for(let i=0;i<n;i+=2){next[cells[i]]=cells[i+1];next[cells[i+1]]=cells[i]}if(next.every((to,i)=>to!==i)){permutation=cells;break}}if(!permutation){const order=Array.from({length:n},(_,i)=>(i+1)%n);permutation=[];for(let i=0;i<n;i+=2){let a=order[i],b=order[i+1];if(a===i||b===i+1){a=order[(i+1)%n];b=order[i]}permutation.push(a,b)}}p.operations=[];for(let i=0;i<n;i+=2)p.operations.push({type:"swap",cells:[permutation[i],permutation[i+1]]});markDirty();renderEditor()}
...
saveLevel=async function(status){try{const l=state.level;if(status==="published"&&l.mode==="find_anachronism"){if((l.differences||[]).length<3)throw new Error("找穿帮关卡至少需要 3 个答案");const incomplete=l.differences.find(x=>!String(x.era||"").trim()||String(x.explanation||"").trim().length<20);if(incomplete)throw new Error(`答案“${incomplete.label||incomplete.id}”需要填写线索和至少20字的线索推理`)}if(status==="published"&&l.mode==="image_puzzle"&&!l.puzzle?.operations?.length)throw new Error("发布拼图关卡前至少配置一组交换");setSave("保存中…");l.instruction=l.mode==="image_puzzle"?"交换错位区域，还原完整画面":`圈出 ${l.differences.length} 个不属于这个年代的物件`;l.level_version=Number(l.level_version||1)+1;const seriesId=$("editor-series")?.value||state.series.id;await api(`/levels/${encodeURIComponent(l.level_id)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({series_id:seriesId,sort_order:state.levelEntry?.sort_order||10,status,runtime_json:l})});setSave("已保存",false);toast(status==="published"?"关卡已发布":"草稿已保存");await loadData();state.series=state.catalog.find(s=>s.id===seriesId);state.levelEntry=state.series?.levels.find(x=>x.id===l.level_id);renderEditor()}catch(e){setSave("保存失败",true);toast(e.message,"error")}};
showPublishModal=function(){const l=state.level,d=l.differences||[],p=l.puzzle,valid=l.mode==="image_puzzle"?!!p?.operations?.length:d.length>=3&&d.every(x=>x.id&&x.label&&x.era&&String(x.explanation||"").trim().length>=20),summary=l.mode==="image_puzzle"?`${p?.rows||0}×${p?.cols||0} 网格，${(p?.operations?.length||0)*2} 个错位块`:`${d.length} 个答案热点`;$("modal-root").innerHTML=`<div class="modal-backdrop"><div class="modal"><div class="modal-header"><h2>发布关卡</h2><button class="btn ghost" onclick="closeModal()">✕</button></div><div class="modal-body"><p><b>${escapeHtml(l.title)}</b></p><div class="checks"><div class="check">✓ 图片资源已配置</div><div class="check">✓ ${summary}</div><div class="check">${valid?"✓":"⚠"} 发布校验</div></div></div><div class="modal-footer"><button class="btn" onclick="closeModal()">取消</button><button class="btn primary" ${valid?"":"disabled"} onclick="closeModal();saveLevel('published')">确认并发布</button></div></div></div>`};
...
const saveLevelWithoutFullPuzzleValidation=saveLevel;
saveLevel=async function(status){if(status==="published"&&state.level?.mode==="image_puzzle"){const p=state.level.puzzle,used=new Set((p?.operations||[]).flatMap(op=>op.cells));if(!p?.operations?.length||used.size!==p.rows*p.cols){toast("发布前必须让每一块都处于错误位置，请点击随机生成","error");return}}return saveLevelWithoutFullPuzzleValidation(status)};
