...
const state={catalog:[],home:{},museum:{title:"大侦探博物馆",subtitle:"",items:[]},assets:[],view:"overview",series:null,level:null,levelEntry:null,selected:0,dirty:false,adding:false,puzzleSelected:-1,puzzleView:"mixed",storyNodeId:"",storyHotspot:0,storyEditorMode:"visual"};
...
async function loadData(){const [catalog,assets]=await Promise.all([api("/catalog"),api("/assets")]);state.catalog=catalog.series||[];state.home=catalog.home||{};state.museum=catalog.museum||{title:"大侦探博物馆",subtitle:"",items:[]};state.assets=assets.items||[]}
...
function levelModeLabel(mode){return mode==="interactive_story"?"互动故事":mode==="image_puzzle"?"拼图":"找穿帮"}
function levelTable(levels,series){
  if(!levels.length)return`<div class="empty">暂无关卡</div>`;
  return `<table class="level-table"><thead><tr><th>预览</th><th>关卡</th><th>玩法</th><th>内容数</th><th>难度</th><th>版本</th><th>状态</th><th></th></tr></thead><tbody>${levels.map(l=>{const sid=series?.id||findSeries(l.id)?.id||"";return `<tr><td><img class="thumb" src="${escapeHtml(l.thumbnail_url)}"></td><td><b>${escapeHtml(l.title)}</b><br><small>${escapeHtml(l.id)}</small></td><td>${levelModeLabel(l.mode)}</td><td>${l.content_count??l.difference_count??0}</td><td>${"◆".repeat(l.difficulty||1)}</td><td>v${l.version}</td><td><span class="badge ${l.status==="published"?"published":"draft"}">${levelStatusLabel(l.status)}</span></td><td style="white-space:nowrap"><div style="display:flex;gap:6px;justify-content:flex-end"><button class="btn small" onclick="openEditorById('${l.id}','${sid}')">编辑</button>${l.status==="published"?`<button class="btn small" onclick="setLevelStatus('${l.id}','disabled')">下架</button>`:""}${l.status==="disabled"?`<button class="btn small" onclick="setLevelStatus('${l.id}','published')">上架</button>`:""}${series?`<button class="btn small" onclick="removeLevelFromSeries('${series.id}','${l.id}')">移出系列</button>`:""}${l.status!=="published"?`<button class="btn small danger" onclick="deleteLevelById('${l.id}')">删除</button>`:""}</div></td></tr>`}).join("")}</tbody></table>`;
}
...
async function removeLevelFromSeries(seriesId,levelId){const s=state.catalog.find(x=>x.id===seriesId),l=(s?.levels||[]).find(x=>x.id===levelId)||state.catalog.flatMap(x=>x.levels||[]).find(x=>x.id===levelId);if(!confirm(`确定把关卡「${l?.title||levelId}」移出系列「${s?.title||seriesId}」吗？\n关卡本身及其在其它系列中的绑定不受影响。`))return;try{await api(`/series/${encodeURIComponent(seriesId)}/levels/${encodeURIComponent(levelId)}`,{method:"DELETE"});await loadData();rerenderAfterCatalogChange();toast("已移出该系列")}catch(e){toast(e.message,"error")}}
...
function renderLibrary(){
  shell("library","素材库");$("view").innerHTML=`<div class="page"><div class="page-header"><div><h1>素材库</h1><p>列表加载轻量缩略图，进入关卡后才加载原图</p></div><button class="btn primary" onclick="$('asset-file').click()">上传素材</button></div><div class="dropzone" onclick="$('asset-file').click()"><b>拖放或点击上传图片 / 音频</b><p>图片 PNG、JPG；音频 MP3、WAV、OGG、M4A，最大 15 MB</p><input id="asset-file" type="file" accept="image/png,image/jpeg,audio/mpeg,audio/wav,audio/ogg,audio/mp4,.mp3,.wav,.ogg,.m4a" hidden onchange="uploadLibraryAsset(this.files[0])"></div><div class="library-grid">${state.assets.map(a=>{const isAudio=a.kind==="audio"||String(a.content_type||"").startsWith("audio");return `<article class="asset-card">${isAudio?`<div class="asset-audio" style="display:flex;align-items:center;justify-content:center;height:120px;font-size:44px;background:#f1f2f4">🎵</div>`:`<img src="${escapeHtml(a.thumbnail_url||a.url)}" loading="lazy">`}<div class="asset-info"><b>${escapeHtml(a.name)}</b><small>${isAudio?"音频":"图片"} · ${formatBytes(a.bytes)}${!isAudio&&a.thumbnail_bytes?` · 缩略图 ${formatBytes(a.thumbnail_bytes)}`:""}</small>${isAudio?`<audio controls preload="none" src="${escapeHtml(a.url)}" style="width:100%;margin-top:8px"></audio>`:""}<div style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap"><button class="btn small" onclick="navigator.clipboard.writeText('${escapeHtml(a.url)}');toast('URL 已复制')">复制 URL</button><button class="btn small" onclick="renameLibraryAsset('${escapeHtml(a.name)}')">改名</button><button class="btn small danger" onclick="deleteLibraryAsset('${escapeHtml(a.name)}')">删除</button></div></div></article>`}).join("")}</div></div>`;
}
...
function navigate(view){if(state.dirty&&!confirm("当前关卡有未保存修改，确定离开吗？"))return;({overview:renderOverview,series:renderSeries,levels:renderLevels,library:renderLibrary,home:renderHomeConfig,museum:renderMuseum,review:()=>renderReview(),feedback:()=>renderPlaceholder("feedback","问题反馈","集中处理玩家上报的问题"),analytics:()=>renderPlaceholder("analytics","数据分析","查看完成率、提示使用与热点误触"),settings:()=>renderSettings()})[view]()}

function homeTextField(key,label){return `<label>${label}<input value="${escapeHtml(state.home[key]||"")}" oninput="state.home['${key}']=this.value"></label>`}
function homeAssetField(key,label){const images=state.assets.filter(a=>a.kind==="image"),value=state.home[key]||"";return `<label>${label}<select onchange="state.home['${key}']=this.value"><option value="">不设置</option>${images.map(a=>`<option value="${escapeHtml(a.url)}" ${a.url===value?"selected":""}>${escapeHtml(a.name||a.asset_id)}</option>`).join("")}</select></label>`}
function renderHomeConfig(){shell("home","首页配置");const seriesOptions=state.catalog.filter(s=>s.enabled!==false).map(s=>`<option value="${escapeHtml(s.id)}" ${s.id===state.home.hero_series_id?"selected":""}>${escapeHtml(s.title||s.id)}</option>`).join("");$("view").innerHTML=`<div class="page"><div class="page-header"><div><h1>首页配置</h1><p>客户端首页内容均从这里读取；章节卡片来自已启用内容系列</p></div><button class="btn primary" onclick="saveHomeConfig()">保存并同步</button></div><section class="card form-stack"><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">${homeTextField("brand_title","品牌标题")}${homeTextField("brand_subtitle","品牌副标题")}${homeTextField("detective_title","侦探称号")}${homeTextField("hero_badge","Hero 标签")}${homeTextField("hero_label","Hero 栏目文案")}${homeTextField("hero_button","Hero 按钮")}</div><label>指定 Hero 章节<select onchange="state.home.hero_series_id=this.value"><option value="">自动选择首个未完成章节</option>${seriesOptions}</select></label><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">${homeTextField("daily_title","今日区域标题")}${homeTextField("daily_description","今日区域说明")}${homeTextField("daily_reward","今日奖励文案")}<label>今日目标<input type="number" min="1" value="${Number(state.home.daily_target)||1}" oninput="state.home.daily_target=Number(this.value)"></label>${homeTextField("worlds_title","章节区域标题")}${homeTextField("worlds_link_text","章节入口文案")}${homeTextField("museum_title","收藏区域标题")}${homeTextField("museum_link_text","博物馆入口文案")}</div><h2 style="margin-top:18px">首页素材</h2><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">${homeAssetField("logo_url","品牌 Logo")}${homeAssetField("header_url","顶部纹理")}${homeAssetField("hero_fallback_url","Hero 无封面占位")}${homeAssetField("collection_placeholder_url","未解锁藏品占位")}</div></section></div>`}
async function saveHomeConfig(){try{await api("/home",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(state.home)});await loadData();renderHomeConfig();toast("首页配置已同步到客户端")}catch(e){toast(e.message,"error")}}

function museumUnlockTarget(item,index){
  if(item.unlock_type==="series_complete")return `<label>解锁系列<select onchange="updateMuseumItem(${index},'unlock_value',this.value)"><option value="">请选择系列</option>${state.catalog.map(s=>`<option value="${escapeHtml(s.id)}" ${s.id===item.unlock_value?"selected":""}>${escapeHtml(s.title||s.id)}（${escapeHtml(s.id)}）</option>`).join("")}</select></label>`;
  if(item.unlock_type==="level_complete")return `<label>解锁关卡<select onchange="updateMuseumItem(${index},'unlock_value',this.value)"><option value="">请选择关卡</option>${state.catalog.map(s=>`<optgroup label="${escapeHtml(s.title||s.id)}">${(s.levels||[]).map(l=>`<option value="${escapeHtml(l.id)}" ${l.id===item.unlock_value?"selected":""}>${escapeHtml(l.title||l.id)}（${escapeHtml(l.id)}）</option>`).join("")}</optgroup>`).join("")}</select></label>`;
  return `<label>解锁目标<input value="无需选择" disabled></label>`;
}
function museumItemRow(item,index){const assets=state.assets.filter(a=>a.kind==="image"),imageUrl=item.image_url||"";return `<section class="card" style="margin-bottom:14px"><div class="page-header" style="margin-bottom:12px"><div><h3 style="margin:0">${escapeHtml(item.name||"新藏品")}</h3><small>${escapeHtml(item.id||"")}</small></div><div class="actions"><button class="btn small" onclick="moveMuseumItem(${index},-1)">↑</button><button class="btn small" onclick="moveMuseumItem(${index},1)">↓</button><button class="btn small danger" onclick="removeMuseumItem(${index})">删除</button></div></div><div class="form-stack"><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><label>藏品 ID<input value="${escapeHtml(item.id||"")}" oninput="updateMuseumItem(${index},'id',this.value)"></label><label>名称<input value="${escapeHtml(item.name||"")}" oninput="updateMuseumItem(${index},'name',this.value)"></label></div><label>说明<textarea oninput="updateMuseumItem(${index},'description',this.value)">${escapeHtml(item.description||"")}</textarea></label><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><label>分组<input value="${escapeHtml(item.group||"")}" oninput="updateMuseumItem(${index},'group',this.value)"></label><label>排序<input type="number" value="${Number(item.sort_order)||0}" oninput="updateMuseumItem(${index},'sort_order',Number(this.value))"></label></div><label>藏品图片<div class="museum-image-field">${imageUrl?`<div class="museum-image-preview"><img src="${escapeHtml(imageUrl)}" alt="${escapeHtml(item.name||"藏品图片")}" loading="lazy" onerror="this.parentElement.classList.add('load-error');this.hidden=true"><span>图片加载失败</span></div>`:`<div class="museum-image-preview empty-preview"><span>尚未选择图片</span></div>`}<select onchange="updateMuseumImage(${index},this.value)"><option value="">请选择素材</option>${assets.map(a=>`<option value="${escapeHtml(a.url)}" ${a.url===imageUrl?"selected":""}>${escapeHtml(a.name||a.asset_id)}</option>`).join("")}</select></div></label><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><label>解锁条件<select onchange="updateMuseumUnlockType(${index},this.value)"><option value="always" ${item.unlock_type==="always"?"selected":""}>直接解锁</option><option value="level_complete" ${item.unlock_type==="level_complete"?"selected":""}>完成指定关卡</option><option value="series_complete" ${item.unlock_type==="series_complete"?"selected":""}>完成指定系列</option></select></label>${museumUnlockTarget(item,index)}</div><label><input type="checkbox" ${item.enabled!==false?"checked":""} onchange="updateMuseumItem(${index},'enabled',this.checked)"> 启用藏品</label></div></section>`}
function renderMuseum(){shell("museum","博物馆");const m=state.museum||{items:[]};$("view").innerHTML=`<div class="page"><div class="page-header"><div><h1>大侦探博物馆</h1><p>配置客户端展示的藏品、分组与解锁条件</p></div><div class="actions"><button class="btn" onclick="addMuseumItem()">＋ 新增藏品</button><button class="btn primary" onclick="saveMuseum()">保存并同步</button></div></div><section class="card form-stack" style="margin-bottom:18px"><label>页面标题<input value="${escapeHtml(m.title||"")}" oninput="state.museum.title=this.value"></label><label>页面副标题<input value="${escapeHtml(m.subtitle||"")}" oninput="state.museum.subtitle=this.value"></label></section><div id="museum-items">${(m.items||[]).map(museumItemRow).join("")||'<div class="empty">尚未配置藏品</div>'}</div></div>`}
function updateMuseumItem(index,key,value){state.museum.items[index][key]=value}
function updateMuseumUnlockType(index,value){state.museum.items[index].unlock_type=value;state.museum.items[index].unlock_value="";renderMuseum()}
function updateMuseumImage(index,value){updateMuseumItem(index,"image_url",value);renderMuseum()}
function addMuseumItem(){state.museum.items.push({id:`collection_${Date.now()}`,name:"新藏品",description:"",group:"未分组",image_url:"",sort_order:(state.museum.items.length+1)*10,enabled:true,unlock_type:"always",unlock_value:""});renderMuseum()}
function removeMuseumItem(index){if(!confirm("确定删除这个藏品配置吗？"))return;state.museum.items.splice(index,1);renderMuseum()}
function moveMuseumItem(index,delta){const next=index+delta;if(next<0||next>=state.museum.items.length)return;const [item]=state.museum.items.splice(index,1);state.museum.items.splice(next,0,item);state.museum.items.forEach((x,i)=>x.sort_order=(i+1)*10);renderMuseum()}
async function saveMuseum(){try{await api("/museum",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(state.museum)});await loadData();renderMuseum();toast("博物馆配置已同步到客户端")}catch(e){toast(e.message,"error")}}
...
function renderEditor(){
  ...
  const modeSelect=$("view").querySelector('select[onchange="changeLevelMode(this.value)"]');if(modeSelect)modeSelect.insertAdjacentHTML("beforeend",'<option value="interactive_story">互动故事</option>');bindHotspotDrag();
}
function renderPuzzleEditor(){
  const l=state.level,p=l.puzzle||(l.puzzle={rows:5,cols:4}),asset=l.assets?.image||{};
  $("view").innerHTML=`<div class="editor"><aside class="editor-left"><div class="editor-head"><h2>${escapeHtml(l.title)}</h2><small>${escapeHtml(l.level_id)} · v${l.level_version}</small></div><div class="panel-section"><div class="panel-title">关卡信息</div><div class="form-stack"><label>标题<input value="${escapeHtml(l.title)}" oninput="updateLevelField('title',this.value)"></label><label>玩法<select onchange="changeLevelMode(this.value)"><option value="find_anachronism">找穿帮</option><option value="image_puzzle" selected>拼图（错位还原）</option></select></label><label>所属系列<select id="editor-series" onchange="markDirty()">${state.catalog.map(s=>`<option value="${s.id}" ${s.id===state.series?.id?"selected":""}>${escapeHtml(s.title)}</option>`).join("")}</select></label><label>难度<select id="level-difficulty" onchange="updateDifficulty(this.value)">${[1,2,3,4,5].map(n=>`<option ${n===(l.difficulty?.total||1)?"selected":""}>${n}</option>`).join("")}</select></label><label>限时（秒，0 为不限时）<input type="number" min="0" max="3600" step="5" value="${configuredTimeLimit(l)}" onchange="updateTimeLimit(this.value)"></label></div></div><div class="panel-section"><div class="panel-title">拼图设置</div><div class="coord-grid"><label>行<input type="number" min="2" max="8" value="${p.rows}" onchange="updatePuzzleGrid('rows',this.value)"></label><label>列<input type="number" min="2" max="8" value="${p.cols}" onchange="updatePuzzleGrid('cols',this.value)"></label></div><p>共 ${p.rows*p.cols} 块 · 玩家每次开局都会随机打乱（每块都不在原位）</p></div></aside><section class="canvas-pane"><div class="canvas-toolbar"><div class="canvas-tools"><span>分割预览</span></div><span>${l.assets?.width||0} × ${l.assets?.height||0}</span></div><div class="canvas-workspace"><canvas id="puzzle-admin-canvas" class="puzzle-admin-canvas" data-src="${escapeHtml(asset.url||asset.local_path||"")}"></canvas></div><footer class="editor-footer"><span id="editor-status">调整行列即可，打乱由客户端随机生成</span><div class="actions"><button class="btn" onclick="showMobilePreview()">手机预览</button><button class="btn" onclick="saveLevel('draft')">保存草稿</button><button class="btn primary" onclick="showPublishModal()">发布关卡</button></div></footer></section><aside class="inspector" id="inspector"><div class="panel-section"><h2>拼图说明</h2><p>管理端只需设置分割的行数与列数。玩家每次进入或重玩时，客户端会随机打乱所有拼块并保证每一块都不在正确位置，完成后记录用时。</p></div></aside></div>`;
  const modeSelect=$("view").querySelector('select[onchange="changeLevelMode(this.value)"]');if(modeSelect)modeSelect.insertAdjacentHTML("beforeend",'<option value="interactive_story">互动故事</option>');drawPuzzleAdmin();
}
function changeLevelMode(mode){if(mode===state.level.mode)return;if(!confirm("切换玩法会重置当前玩法配置，是否继续？")){renderEditor();return}delete state.level.differences;delete state.level.puzzle;delete state.level.story;if(mode==="image_puzzle"){state.level.puzzle={rows:5,cols:4,time_limit_seconds:configuredTimeLimit(state.level)};state.level.instruction="随机打乱拼块，还原完整画面"}else if(mode==="interactive_story"){state.level.story=defaultStory();state.level.assets=state.level.assets||{};state.level.instruction="调查场景、收集证据并推进故事"}else{state.level.differences=[];state.level.instruction="圈出不属于这个年代的物件"}state.level.mode=mode;state.puzzleSelected=-1;markDirty();renderEditor()}

function defaultStory(){return{start_node:"intro",nodes:{intro:{type:"scene",title:"故事开始",text:"在这里填写开场内容。",next:"ending"},ending:{type:"ending",title:"故事结束",text:"案件已经解决。",ending_id:"solved"}}}}
function storyJsonChanged(value){try{state.level.story=JSON.parse(value);$("story-json-error").textContent="JSON 格式正确";markDirty()}catch(e){$("story-json-error").textContent=`JSON 错误：${e.message}`}}
function storyNode(){return state.level?.story?.nodes?.[state.storyNodeId]}
function storyImageUrl(node=storyNode()){return node?.image?.url||node?.image?.local_path||""}
function selectStoryNode(id){state.storyNodeId=id;state.storyHotspot=0;state.storyEditorMode="visual";renderStoryEditor()}
function selectStoryHotspot(i){state.storyHotspot=i;document.querySelectorAll(".story-hotspot").forEach((x,n)=>x.classList.toggle("selected",n===i));document.querySelectorAll(".story-hotspot-row").forEach((x,n)=>x.classList.toggle("active",n===i));$("inspector").innerHTML=storyHotspotInspector()}
function storyHotspots(){const node=storyNode();return(node?.hotspots||[]).map((spot,i)=>{const radius=Math.max(3,Number(spot.radius||.06)*100);return`<button class="hotspot story-hotspot ${i===state.storyHotspot?"selected":""}" data-index="${i}" style="left:${Number(spot.x||0)*100}%;top:${Number(spot.y||0)*100}%;width:${radius*2}%;aspect-ratio:1" title="${escapeHtml(spot.label||spot.id)}">${i+1}</button>`}).join("")}
function storyHotspotInspector(){const node=storyNode(),spot=node?.hotspots?.[state.storyHotspot];if(!node||node.type!=="hotspot")return`<div class="empty">选择 hotspot 节点后可在图片上编辑命中区域</div>`;if(!spot)return`<div class="panel-section"><button class="btn primary" style="width:100%" onclick="addStoryHotspot()">＋ 添加热点</button></div>`;return`<div class="editor-head"><h2>热点 ${state.storyHotspot+1}</h2><small>${escapeHtml(state.storyNodeId)} / ${escapeHtml(spot.id||"")}</small></div><div class="panel-section"><div class="form-stack"><label>热点 ID<input value="${escapeHtml(spot.id||"")}" oninput="updateStoryHotspot('id',this.value)"></label><label>点亮说明<input value="${escapeHtml(spot.label||"")}" oninput="updateStoryHotspot('label',this.value)"></label><label>详细说明<textarea oninput="updateStoryHotspot('description',this.value)">${escapeHtml(spot.description||spot.explanation||"")}</textarea></label><label>证据 ID<input value="${escapeHtml(spot.evidence_id||"")}" oninput="updateStoryHotspot('evidence_id',this.value)"></label></div></div><div class="panel-section"><div class="panel-title">位置与点击范围</div><div class="coord-grid"><label>X<input type="number" min="0" max="1" step=".001" value="${Number(spot.x||0).toFixed(3)}" onchange="updateStoryHotspot('x',Number(this.value),true)"></label><label>Y<input type="number" min="0" max="1" step=".001" value="${Number(spot.y||0).toFixed(3)}" onchange="updateStoryHotspot('y',Number(this.value),true)"></label><label>半径<input type="number" min=".01" max=".3" step=".005" value="${Number(spot.radius||.06).toFixed(3)}" onchange="updateStoryHotspot('radius',Number(this.value),true)"></label></div><label style="display:grid;gap:6px;margin-top:12px">拖动调整半径<input type="range" min=".01" max=".3" step=".005" value="${Number(spot.radius||.06)}" oninput="updateStoryHotspot('radius',Number(this.value),true)"></label><small>可直接拖动图片上的圆；点击图片空白处会把当前圆移动到点击位置。</small></div><div class="panel-section"><button class="btn danger" style="width:100%" onclick="removeStoryHotspot()">删除热点</button></div>`}
function updateStoryHotspot(key,value,redraw=false){const spot=storyNode()?.hotspots?.[state.storyHotspot];if(!spot)return;spot[key]=value;if(key==="description")delete spot.explanation;markDirty();if(redraw)renderStoryEditor()}
function addStoryHotspot(){const node=storyNode();if(!node||node.type!=="hotspot")return;node.hotspots=node.hotspots||[];node.hotspots.push({id:`hotspot_${node.hotspots.length+1}`,x:.5,y:.5,radius:.06,label:"新线索",description:"",evidence_id:""});state.storyHotspot=node.hotspots.length-1;markDirty();renderStoryEditor()}
function removeStoryHotspot(){const node=storyNode();if(!node?.hotspots?.length||!confirm("确定删除这个故事热点吗？"))return;node.hotspots.splice(state.storyHotspot,1);state.storyHotspot=Math.max(0,state.storyHotspot-1);markDirty();renderStoryEditor()}
function storyStageClick(event){if(event.target.classList.contains("story-hotspot"))return;const img=$("story-node-image"),spot=storyNode()?.hotspots?.[state.storyHotspot];if(!img||!spot)return;const rect=img.getBoundingClientRect();if(event.clientX<rect.left||event.clientX>rect.right||event.clientY<rect.top||event.clientY>rect.bottom)return;spot.x=+Math.max(0,Math.min(1,(event.clientX-rect.left)/rect.width)).toFixed(4);spot.y=+Math.max(0,Math.min(1,(event.clientY-rect.top)/rect.height)).toFixed(4);markDirty();renderStoryEditor()}
function bindStoryHotspotDrag(){document.querySelectorAll(".story-hotspot").forEach(el=>el.onpointerdown=e=>{e.stopPropagation();const i=Number(el.dataset.index);selectStoryHotspot(i);el.setPointerCapture(e.pointerId);el.onpointermove=move=>{const img=$("story-node-image"),rect=img.getBoundingClientRect(),spot=storyNode().hotspots[i];spot.x=Math.max(0,Math.min(1,(move.clientX-rect.left)/rect.width));spot.y=Math.max(0,Math.min(1,(move.clientY-rect.top)/rect.height));el.style.left=`${spot.x*100}%`;el.style.top=`${spot.y*100}%`;markDirty()};el.onpointerup=()=>{el.onpointermove=null;$("inspector").innerHTML=storyHotspotInspector()}})}
function renderStoryEditor(){const l=state.level,story=l.story||(l.story=defaultStory()),nodes=Object.entries(story.nodes||{});if(!story.nodes?.[state.storyNodeId])state.storyNodeId=(nodes.find(([,n])=>n.type==="hotspot")||nodes[0]||[])[0]||"";const node=storyNode(),visual=state.storyEditorMode!=="json",image=storyImageUrl(node);$("view").innerHTML=`<div class="editor"><aside class="editor-left"><div class="editor-head"><h2>${escapeHtml(l.title)}</h2><small>${escapeHtml(l.level_id)} · v${l.level_version}</small></div><div class="panel-section"><div class="panel-title">剧情节点 ${nodes.length}</div><div class="answer-list">${nodes.map(([id,n])=>`<div class="answer-item ${id===state.storyNodeId?"active":""}" onclick="selectStoryNode('${escapeHtml(id)}')"><span class="answer-number">${escapeHtml(n.type.slice(0,1).toUpperCase())}</span><div><b>${escapeHtml(n.title||n.speaker||id)}</b><br><small>${escapeHtml(id)} · ${escapeHtml(n.type)}</small></div></div>`).join("")}</div></div></aside><section class="canvas-pane"><div class="canvas-toolbar"><div class="canvas-tools"><button class="btn small ${visual?"primary":""}" onclick="state.storyEditorMode='visual';renderStoryEditor()">可视化</button><button class="btn small ${!visual?"primary":""}" onclick="state.storyEditorMode='json';renderStoryEditor()">JSON</button>${visual&&node?.type==="hotspot"?`<button class="btn small" onclick="addStoryHotspot()">＋ 添加热点</button>`:""}</div><span>${escapeHtml(state.storyNodeId)} · ${escapeHtml(node?.type||"")}</span></div>${visual?`<div class="canvas-workspace">${image?`<div id="story-image-stage" class="image-stage" onclick="storyStageClick(event)"><img id="story-node-image" src="${escapeHtml(image)}" draggable="false">${node?.type==="hotspot"?storyHotspots():""}</div>`:`<div class="empty">此节点没有图片，热点节点需要先在 JSON 中配置 image 资源</div>`}</div>`:`<div style="padding:20px;flex:1;background:#f5f6f8"><textarea spellcheck="false" style="width:100%;height:calc(100% - 35px);font:13px/1.55 Consolas,monospace;resize:none" oninput="storyJsonChanged(this.value)">${escapeHtml(JSON.stringify(story,null,2))}</textarea><small id="story-json-error">高级模式：可编辑全部故事节点字段。</small></div>`}<footer class="editor-footer"><span id="editor-status">${node?.type==="hotspot"?`拖动圆或点击图片定位 · ${node.hotspots?.length||0} 个热点`:"选择 hotspot 节点可编辑命中区域"}</span><div class="actions"><button class="btn" onclick="saveLevel('draft')">保存草稿</button><button class="btn primary" onclick="saveLevel('published')">发布故事</button></div></footer></section><aside class="inspector" id="inspector">${visual?storyHotspotInspector():`<div class="panel-section"><h2>故事 JSON</h2><p>Web 与微信小游戏读取同一份节点数据。完成 JSON 修改后可切回可视化检查热点坐标。</p></div>`}</aside></div>`;if(visual&&node?.type==="hotspot")bindStoryHotspotDrag()}
...
function showSeriesModal(id=""){const s=state.catalog.find(x=>x.id===id)||{id:"",title:"",description:"",mode:"find_anachronism",cover_url:"",sort_order:10,enabled:true,min_web_version:"",min_wechat_version:""};$("modal-root").innerHTML=`<div class="modal-backdrop"><form class="modal" onsubmit="saveSeries(event)"><div class="modal-header"><h2>${id?"编辑":"新建"}系列</h2><button type="button" class="btn ghost" onclick="closeModal()">✕</button></div><div class="modal-body form-stack"><label>系列 ID<input id="series-id" value="${escapeHtml(s.id)}" ${id?"readonly":""} required></label><label>系列名称<input id="series-title" value="${escapeHtml(s.title)}" required></label><label>题材<select id="series-mode"><option value="find_anachronism" ${s.mode==="find_anachronism"?"selected":""}>找穿帮</option><option value="image_puzzle" ${s.mode==="image_puzzle"?"selected":""}>拼图</option><option value="interactive_story" ${s.mode==="interactive_story"?"selected":""}>互动故事</option></select></label><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><label>Web 最低版本<input id="series-min-web" value="${escapeHtml(s.min_web_version||"")}" placeholder="例如 0.3.0，留空不限"></label><label>微信最低版本<input id="series-min-wechat" value="${escapeHtml(s.min_wechat_version||"")}" placeholder="例如 0.2.0，留空不限"></label></div><label>说明<textarea id="series-description">${escapeHtml(s.description)}</textarea></label><label>封面 URL<input id="series-cover" value="${escapeHtml(s.cover_url)}"></label><label><input id="series-enabled" type="checkbox" ${s.enabled?"checked":""}> 启用系列</label></div><div class="modal-footer"><button type="button" class="btn" onclick="closeModal()">取消</button><button class="btn primary">保存系列</button></div></form></div>`}
async function saveSeries(event){event.preventDefault();const existing=state.catalog.find(x=>x.id===$("series-id").value);const nextOrder=Math.max(0,...state.catalog.filter(x=>x.id!=="daily_task").map(x=>Number(x.sort_order)||0))+10;try{await api("/series",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:$("series-id").value,title:$("series-title").value,mode:$("series-mode").value,min_web_version:$("series-min-web").value.trim(),min_wechat_version:$("series-min-wechat").value.trim(),description:$("series-description").value,cover_url:$("series-cover").value,sort_order:existing?.sort_order??nextOrder,enabled:$("series-enabled").checked})});closeModal();await loadData();renderSeries();toast("系列已保存")}catch(e){toast(e.message,"error")}}
...
function assetContentType(file){if(file.type)return file.type;const ext=(String(file.name||"").toLowerCase().match(/\.(png|jpe?g|mp3|wav|ogg|m4a)$/)||[])[1]||"";return {png:"image/png",jpg:"image/jpeg",jpeg:"image/jpeg",mp3:"audio/mpeg",wav:"audio/wav",ogg:"audio/ogg",m4a:"audio/mp4"}[ext]||"application/octet-stream"}
async function uploadLibraryAsset(file){if(!file)return;try{const id=`asset_${Date.now()}`,asset=await api(`/assets/${id}`,{method:"POST",headers:{"Content-Type":assetContentType(file)},body:file});toast("素材上传成功");await loadData();renderLibrary();return asset}catch(e){toast(e.message,"error")}}
async function deleteLibraryAsset(name){if(!name)return;if(!confirm(`确定删除素材「${name}」吗？\n仅当没有任何关卡或系列引用它时才能删除，且不可恢复。`))return;try{await api(`/assets/${encodeURIComponent(name)}`,{method:"DELETE"});await loadData();renderLibrary();toast("素材已删除")}catch(e){toast(/referenced|IN_USE/i.test(e.message||"")?"该素材已被关卡或系列引用，无法删除":(e.message||"删除失败"),"error")}}
async function renameLibraryAsset(name){if(!name)return;const base=String(name).replace(/\.[^.]+$/,"");const input=prompt("输入新的素材名称（仅字母、数字、- 和 _）：",base);if(input==null)return;const next=String(input).trim();if(!next||next===base)return;try{await api(`/assets/${encodeURIComponent(name)}/rename`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:next})});await loadData();renderLibrary();toast("素材已改名，引用已同步更新")}catch(e){toast(/TAKEN/i.test(e.message||"")?"已存在同名素材":(e.message||"改名失败"),"error")}}
...
renderEditor=function(){
  ...
  const leftAside=document.querySelector(".editor-left");
  if(leftAside){
    const music=state.level.assets&&state.level.assets.music,musicSection=document.createElement("div");musicSection.className="panel-section";
    musicSection.innerHTML=`<div class="panel-title">关卡音乐</div><div class="asset-picker-summary">${music&&music.url?`<small>🎵 ${escapeHtml(music.asset_id||"自定义音乐")}</small><audio controls preload="none" src="${escapeHtml(music.url)}" style="width:100%;margin-top:8px"></audio>`:'<div class="empty" style="padding:12px">未设置 · 进入关卡播放默认背景音乐</div>'}</div><div class="actions"><button class="btn small" onclick="openLevelMusicPicker()">从素材库选择</button><button class="btn small primary" onclick="$('level-music-file').click()">上传音乐</button>${music&&music.url?`<button class="btn small danger" onclick="clearLevelMusicAsset()">清除</button>`:""}</div><input id="level-music-file" type="file" accept="audio/mpeg,audio/wav,audio/ogg,audio/mp4,.mp3,.wav,.ogg,.m4a" hidden onchange="uploadLevelMusic(this.files[0])">`;
    leftAside.appendChild(musicSection);
    const kn=state.level.background_knowledge||"",knSection=document.createElement("div");knSection.className="panel-section";
    knSection.innerHTML=`<div class="panel-title">背景知识</div><label style="display:block"><textarea rows="6" placeholder="写一段这个关卡的背景知识；玩家可在关卡内点按钮查看，通关后也会展示（过长可滚动）" oninput="updateLevelField('background_knowledge',this.value)" style="width:100%;box-sizing:border-box">${escapeHtml(kn)}</textarea></label><p><small>${kn?`${kn.length} 字`:"未配置 · 游戏内不显示背景知识按钮"}</small></p>`;
    leftAside.appendChild(knSection);
  }
...
function levelAudioAssets(){return state.assets.filter(a=>a.kind==="audio"||String(a.content_type||"").startsWith("audio"))}
function openLevelMusicPicker(){closeLevelMusicPicker();const audios=levelAudioAssets();document.body.insertAdjacentHTML("beforeend",`<div id="level-music-dialog" class="cover-dialog-backdrop" onclick="if(event.target===this)closeLevelMusicPicker()"><section class="cover-dialog" role="dialog" aria-modal="true"><header class="modal-header"><div><h2>选择关卡音乐</h2><small>从素材库选择一段音频作为该关卡背景音乐</small></div><button type="button" class="btn ghost" onclick="closeLevelMusicPicker()">✕</button></header><div class="cover-dialog-toolbar"><input type="search" placeholder="搜索音频…" oninput="filterLevelMusic(this.value)"><button class="btn small primary" onclick="closeLevelMusicPicker();$('level-music-file').click()">上传新音频</button></div><div class="cover-dialog-scroll">${audios.map((a,i)=>`<div class="level-music-option" data-search="${escapeHtml(String(a.name||"").toLowerCase())}" style="padding:10px 4px;border-bottom:1px solid #eee"><b>${escapeHtml(a.name||a.asset_id)}</b> <small>${formatBytes(a.bytes||0)}</small><audio controls preload="none" src="${escapeHtml(a.url)}" style="width:100%;margin:6px 0"></audio><button class="btn small primary" onclick="selectLevelMusicAsset(${i})">选择这段</button></div>`).join("")||'<div class="empty">素材库暂无音频，请先在素材库或此处上传</div>'}</div></section></div>`)}
function closeLevelMusicPicker(){$("level-music-dialog")?.remove()}
function filterLevelMusic(value){const q=String(value||"").trim().toLowerCase();document.querySelectorAll(".level-music-option").forEach(x=>x.hidden=!x.dataset.search.includes(q))}
function selectLevelMusicAsset(index){const a=levelAudioAssets()[index];if(!a)return;state.level.assets=state.level.assets||{};state.level.assets.music={asset_id:a.asset_id||String(a.name||"asset_music").replace(/\.[^.]+$/,""),url:a.url,sha256:a.sha256||"",bytes:Number(a.bytes||1),content_type:a.content_type||"audio/mpeg"};closeLevelMusicPicker();markDirty();renderEditor();toast("关卡音乐已设置")}
async function uploadLevelMusic(file){if(!file)return;const ct=assetContentType(file);if(!ct.startsWith("audio"))return toast("仅支持 MP3 / WAV / OGG / M4A","error");if(file.size>15*1024*1024)return toast("音频不能超过 15 MB","error");try{const asset=await api(`/assets/level_music_${Date.now()}`,{method:"POST",headers:{"Content-Type":ct},body:file});state.level.assets=state.level.assets||{};state.level.assets.music={asset_id:asset.asset_id,url:asset.url,sha256:asset.sha256,bytes:asset.bytes,content_type:asset.content_type};await loadData();markDirty();renderEditor();toast("音乐上传并设置成功")}catch(e){toast(e.message,"error")}}
function clearLevelMusicAsset(){if(state.level.assets)delete state.level.assets.music;markDirty();renderEditor();toast("已清除，使用默认背景音乐")}
...
saveLevel=async function(status){try{const l=state.level;if(status==="published"&&l.mode==="find_anachronism"){if((l.differences||[]).length<3)throw new Error("找穿帮关卡至少需要 3 个答案");const incomplete=l.differences.find(x=>!String(x.era||"").trim()||String(x.explanation||"").trim().length<20);if(incomplete)throw new Error(`答案“${incomplete.label||incomplete.id}”需要填写线索和至少20字的线索推理`)}if(status==="published"&&l.mode==="image_puzzle"&&!(l.puzzle?.rows>=2&&l.puzzle?.cols>=2))throw new Error("发布拼图关卡前请设置有效的行列数");if(status==="published"&&l.mode==="interactive_story"&&(!l.story?.start_node||!l.story?.nodes?.[l.story.start_node]))throw new Error("互动故事需要有效的起始节点");if(l.mode==="image_puzzle")l.puzzle.time_limit_seconds=configuredTimeLimit(l);setSave("保存中…");if(l.mode==="image_puzzle")l.instruction="随机打乱拼块，还原完整画面";else if(l.mode==="find_anachronism")l.instruction=`圈出 ${(l.differences||[]).length} 个不属于这个年代的物件`;l.level_version=Number(l.level_version||1)+1;const seriesId=$("editor-series")?.value||state.series.id;await api(`/levels/${encodeURIComponent(l.level_id)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({series_id:seriesId,sort_order:state.levelEntry?.sort_order||10,status,runtime_json:l})});setSave("已保存",false);toast(status==="published"?"关卡已发布":"草稿已保存");await loadData();state.series=state.catalog.find(s=>s.id===seriesId);state.levelEntry=state.series?.levels.find(x=>x.id===l.level_id);renderEditor()}catch(e){setSave("保存失败",true);toast(e.message,"error")}};
showPublishModal=function(){const l=state.level,d=l.differences||[],p=l.puzzle,nodes=Object.keys(l.story?.nodes||{}),valid=l.mode==="interactive_story"?!!(l.story?.start_node&&l.story?.nodes?.[l.story.start_node]&&nodes.length):l.mode==="image_puzzle"?!!(p&&p.rows>=2&&p.cols>=2):d.length>=3&&d.every(x=>x.id&&x.label&&x.era&&String(x.explanation||"").trim().length>=20),seconds=configuredTimeLimit(l),limitText=seconds>0?`限时 ${seconds} 秒`:"不限时",summary=l.mode==="interactive_story"?`${nodes.length} 个剧情节点 · 起点 ${l.story?.start_node||"未设置"}`:l.mode==="image_puzzle"?`${p?.rows||0}×${p?.cols||0} 网格，共 ${(p?.rows||0)*(p?.cols||0)} 块（客户端随机打乱） · ${limitText}`:`${d.length} 个答案热点 · ${limitText}`;$("modal-root").innerHTML=`<div class="modal-backdrop"><div class="modal"><div class="modal-header"><h2>发布关卡</h2><button class="btn ghost" onclick="closeModal()">✕</button></div><div class="modal-body"><p><b>${escapeHtml(l.title)}</b></p><div class="checks"><div class="check">✓ 图片资源已配置</div><div class="check">✓ ${summary}</div><div class="check">${valid?"✓":"⚠"} 发布校验</div></div></div><div class="modal-footer"><button class="btn" onclick="closeModal()">取消</button><button class="btn primary" ${valid?"":"disabled"} onclick="closeModal();saveLevel('published')">确认并发布</button></div></div></div>`};
...
$("refresh-data").onclick=async()=>{try{$("save-state").textContent="刷新中…";await loadData();rerenderAfterCatalogChange();$("save-state").textContent="刚刚同步";toast("生产数据已刷新")}catch(e){$("save-state").textContent="刷新失败";toast(e.message,"error")}};
