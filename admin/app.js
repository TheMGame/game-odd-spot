...
function randomizePuzzle(){const p=state.level.puzzle,n=p.rows*p.cols;if(n%2){toast("全错位配置要求网格总数为偶数，请调整行或列","error");return}const cells=Array.from({length:n},(_,i)=>i);for(let i=cells.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[cells[i],cells[j]]=[cells[j],cells[i]]}p.operations=[];for(let i=0;i<n;i+=2)p.operations.push({type:"swap",cells:[cells[i],cells[i+1]]});markDirty();renderEditor()}
