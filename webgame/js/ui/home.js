...
function todayDailyLevels(app) { const series = app.seriesById('daily_task'), today = app.businessDate(); return series && Array.isArray(series.levels) ? series.levels.filter((level) => String(level.available_date || '') === today) : [] }
...
function renderHome(app) {
  ...
  r.text(`Lv.${level}`, 408, 58 + top, 27, '#f3d899', 'left', 'bold'); r.text(home.detective_title || '', 493, 58 + top, 24, '#fff8e9'); r.progress(408, 91 + top, 232, 12, exp, 520); r.text(`${exp}/520 EXP`, 524, 119 + top, 18, '#efe5d0', 'center')
  ...
  const dailyLevels = todayDailyLevels(app)
  if (dailyLevels.length) {
    const dailyTarget = dailyLevels.length, dailyDone = dailyLevels.filter((level) => app.isLevelCompleted(level)).length, daily = { x: 42, y, w: 996, h: 128 }; r.rect(daily.x, daily.y, daily.w, daily.h, '#f7eddc', 18, '#decaa5', 2); icon(r, 'search', daily.x + 55, daily.y + 62, '#664d2b'); r.text(home.daily_title || '', daily.x + 105, daily.y + 64, 32, '#211e1b', 'left', 'bold'); r.text(home.daily_description || '', daily.x + 318, daily.y + 38, 20, '#4b4032'); r.progress(daily.x + 318, daily.y + 72, 250, 12, dailyDone, dailyTarget); r.text(`${dailyDone} / ${dailyTarget}`, daily.x + 582, daily.y + 78, 20, '#2d2923'); r.button('daily', { x: daily.x + 815, y: daily.y + 30, w: 155, h: 70 }, home.hero_button || '', { fill: '#162236', border: '#344155', size: 22, radius: 12 }); y += 172
  }
  r.text(home.worlds_title || '', 45, y, 32, '#28231d', 'left', 'bold'); r.text(`${home.worlds_link_text || ''}  ›`, 1018, y, 22, '#786a58', 'right'); y += 42
  ...
  worldEntries.forEach(({ series, title, subtitle }, index) => { const col = index % 3, row = Math.floor(index / 3), rect = { x: 42 + col * 332, y: y + row * 282, w: 310, h: 258 }; r.rect(rect.x, rect.y, rect.w, rect.h, '#1a2633', 18, '#d4b975', 2); const cover = app.covers[series.id]; if (cover) r.image(cover, rect, 'cover'); r.gradientRect(rect.x, rect.y + 125, rect.w, 133, [[0, 'rgba(8,13,18,0)'], [1, 'rgba(7,12,18,.88)']], 16); r.text(title, rect.x + 18, rect.y + 170, 29, '#fff4d8', 'left', 'bold', rect.w - 36); r.text(subtitle, rect.x + 18, rect.y + 205, 18, '#fff', 'left', 'normal', rect.w - 36); const p = progress(app, series); icon(r, 'star', rect.x + 31, rect.y + 235, '#f4c14c'); r.text(`${p.done * 3} / ${p.total * 3}`, rect.x + 52, rect.y + 237, 18, '#fff'); r.register(`series:${series.id}`, rect) })
  ...
  const configuredMuseum = app.catalogData && app.catalogData.museum, recentItems = configuredMuseum && Array.isArray(configuredMuseum.items) ? configuredMuseum.items.filter(item => item.unlocked).slice(0, 3) : []
  const museum = { x: 42, y, w: 996, h: 150 }; r.rect(museum.x, museum.y, museum.w, museum.h, '#f2e2c4', 16, '#dcc59d', 2)
  recentItems.forEach((item, index) => { const x = museum.x + 14 + index * 326, image = app.covers[`museum:${item.id}`] || art.collection; if (image) r.image(image, { x, y: museum.y + 12, w: 112, h: 126 }, 'contain'); r.text(item.name || '', x + 126, museum.y + 55, 23, '#2d261f', 'left', 'bold', 185); r.text(item.group || '', x + 126, museum.y + 91, 17, '#746554', 'left', 'normal', 185) })
  if (!recentItems.length) r.text('尚未获得藏品', museum.x + museum.w / 2, museum.y + 76, 23, '#746554', 'center')
