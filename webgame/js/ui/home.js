...
function seriesFor(app, id, title) { const list = app.enabledSeries(); return list.find((s) => String(s.world_id || s.id) === id) || list.find((s) => String(s.title || s.display_name || '').includes(title.slice(0, 2))) || null }
...
function continueCase(app) { const series = seriesFor(app, 'ancient_china', '神韵华章') || app.enabledSeries()[0]; return series ? { series, level: (series.levels || []).find((level) => !app.isLevelCompleted(level)) || (series.levels || [])[0], stat: progress(app, series) } : null }
...
function renderHome(app) {
  const r = app.renderer, c = r.ctx, h = r.height, top = r.safeTop, art = app.homeArt || {}, scroll = app.scroll.home || 0
  ...
  r.text('错位大侦探', 154, 67 + top, 45, '#f1d28e', 'left', 'bold'); r.text('ODD SPOT DETECTIVE', 157, 112 + top, 14, '#e4c879', 'left', 'bold')
  const stats = app.homeStats || { completed: 0, total: 0 }, exp = Math.min(520, 280 + stats.completed * 15), level = Math.max(1, Math.floor(exp / 45))
  r.text(`Lv.${level}`, 408, 58 + top, 27, '#f3d899', 'left', 'bold'); r.text('敏锐侦探', 493, 58 + top, 24, '#fff8e9'); r.progress(408, 91 + top, 232, 12, exp, 520); r.text(`${exp}/520 EXP`, 524, 119 + top, 18, '#efe5d0', 'center')
  icon(r, 'star', 724, 74 + top, '#f5c44c'); r.text(String(stats.completed * 3), 757, 75 + top, 30, '#fff', 'left', 'bold'); r.headerIconButton('settings', 931, 30 + top, 82, 'settings')
  ...
  r.rect(hero.x + 4, hero.y + 4, 210, 58, '#a83a2d'); r.text('继续调查', hero.x + 108, hero.y + 34, 28, '#fff5df', 'center', 'bold'); r.text('神韵华章 · 第一卷', hero.x + 42, hero.y + 128, 24, '#f5e9ce')
  r.text(active && active.series ? (active.series.chapter_name || active.series.title || active.series.display_name || '洛神赋') : '洛神赋', hero.x + 42, hero.y + 194, 58, '#fff4d6', 'left', 'bold', 470); r.text('翩若惊鸿，婉若游龙', hero.x + 42, hero.y + 250, 25, '#eee4d2')
  const done = active ? active.stat.done : 0, total = active && active.stat.total ? active.stat.total : 8; r.text(`第 ${Math.min(done + 1, total)} / ${total} 案`, hero.x + 42, hero.y + 316, 25, '#fff'); icon(r, 'star', hero.x + 250, hero.y + 316, '#f5c44c'); r.text(`${done * 3} / ${total * 3}`, hero.x + 280, hero.y + 316, 25, '#f1d28e'); r.progress(hero.x + 42, hero.y + 353, 340, 15, done, total); r.text(`${Math.round(done / Math.max(total, 1) * 100)}%`, hero.x + 398, hero.y + 361, 22, '#fff')
  r.button('continueCase', { x: hero.x + 40, y: hero.y + 398, w: 310, h: 78 }, '继续调查  ▶', { fill: '#efc86f', border: '#b58a39', color: '#172233', size: 30, weight: 'bold', radius: 16 }); y += 540
  const daily = { x: 42, y, w: 996, h: 128 }; r.rect(daily.x, daily.y, daily.w, daily.h, '#f7eddc', 18, '#decaa5', 2); icon(r, 'search', daily.x + 55, daily.y + 62, '#664d2b'); r.text('今日案件', daily.x + 105, daily.y + 64, 32, '#211e1b', 'left', 'bold'); r.text('再完成 2 案，获得《洛神赋卷》', daily.x + 318, daily.y + 38, 20, '#4b4032'); r.progress(daily.x + 318, daily.y + 72, 250, 12, 1, 3); r.text('1 / 3', daily.x + 582, daily.y + 78, 20, '#2d2923'); r.text('奖励：EXP +30', daily.x + 664, daily.y + 63, 21, '#362d24'); r.button('daily', { x: daily.x + 815, y: daily.y + 30, w: 155, h: 70 }, '前往挑战', { fill: '#162236', border: '#344155', size: 22, radius: 12 })
  y += 172; r.text('案件世界', 45, y, 32, '#28231d', 'left', 'bold'); r.text('全部世界  ›', 1018, y, 22, '#786a58', 'right'); y += 42
  WORLDS.forEach(([id, title, subtitle, capacity], index) => { const col = index % 3, row = Math.floor(index / 3), rect = { x: 42 + col * 332, y: y + row * 282, w: 310, h: 258 }; r.rect(rect.x, rect.y, rect.w, rect.h, '#1a2633', 18, '#d4b975', 2); if (art[id]) r.image(art[id], rect, 'cover'); r.gradientRect(rect.x, rect.y + 125, rect.w, 133, [[0, 'rgba(8,13,18,0)'], [1, 'rgba(7,12,18,.88)']], 16); r.text(title, rect.x + 18, rect.y + 170, 29, '#fff4d8', 'left', 'bold', rect.w - 36); r.text(subtitle, rect.x + 18, rect.y + 205, 18, '#fff'); const series = seriesFor(app, id, title), p = progress(app, series); icon(r, 'star', rect.x + 31, rect.y + 235, '#f4c14c'); r.text(`${p.done * 3} / ${p.total ? p.total * 3 : capacity}`, rect.x + 52, rect.y + 237, 18, '#fff'); if (series) r.register(`series:${series.id}`, rect) })
  y += 2 * 282 + 25; r.text('最近获得', 45, y, 32, '#28231d', 'left', 'bold'); r.text('我的博物馆  ›', 1018, y, 22, '#786a58', 'right'); y += 38
  ...
  const museum = { x: 42, y, w: 996, h: 150 }; r.rect(museum.x, museum.y, museum.w, museum.h, '#f2e2c4', 16, '#dcc59d', 2); const recentImage = recent && app.covers[`museum:${recent.id}`] || art.scroll; if (recentImage) r.image(recentImage, { x: museum.x + 14, y: museum.y + 8, w: 132, h: 132 }, 'contain'); r.text(recent ? recent.name : '暂无收藏', museum.x + 150, museum.y + 55, 25, '#2d261f', 'left', 'bold'); r.text(recent ? '已收集 · 珍藏于博物馆' : '完成案件章节后解锁藏品', museum.x + 150, museum.y + 93, 18, '#746554')
