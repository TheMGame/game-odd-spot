...
class OddSpotApp {
  ...
  renderHome() {
    ...
    this.enabledSeries().forEach((series, index) => {
      ...
      if (this.covers[series.id]) r.image(this.covers[series.id], { x: rect.x + 2, y: rect.y + 2, w: rect.w - 4, h: rect.h - 4 }, 'cover')
  ...
  renderSettings() {
    ...
    r.text(this.i18n.t('language'), 70, y + 555, 28, COLORS.text); r.button('language', { x: 680, y: y + 512, w: 320, h: 78 }, this.preferences.data.locale === 'zh-CN' ? '简体中文' : 'English', { fill: COLORS.navy, border: COLORS.navy, size: 24 }); y += 650
  ...
  onWheel(event) {
    ...
    if (this.knowledgeScrollActive()) {
      event.preventDefault()
      this.scroll.knowledge = clamp((this.scroll.knowledge || 0) + event.deltaY * 0.6, 0, this.knowledgeMaxScroll || 0)
      return
    }
    const scrollableScenes = ['home', 'museum', 'levels', 'settings']
