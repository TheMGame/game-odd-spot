...
class OddSpotApp {
  ...
  async loadHomeBunny() {
    ...
    const files = Array.from({ length: 20 }, (_, index) => `frame_${String(index * 5).padStart(4, '0')}.png`)
  ...
  bindEvents() {
    ...
    this.canvas.addEventListener('pointerdown', (event) => {
      ...
      this.pointers.set(event.pointerId, pointerLike(event))
    ...
    if (window.visualViewport) window.visualViewport.addEventListener('resize', () => this.renderer.resize())
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) { this.audio.pause(); if (this.game) this.saveAttempt() }
  ...
  showLogin() { this.scene = 'login'; this.status = ''; this.error = ''; this.modal = ''; this.loginView.show() }
  ...
  movePuzzle(source,target) { const p=this.game.puzzle,next=movePuzzleGroup(p.order,p.rows,p.cols,source,target);p.selectedCell=-1;if(!next){this.status='这个块组无法放到该位置';return false}const before=puzzleGroups(p.order,p.rows,p.cols).length;p.order=next;p.moves++;const after=puzzleGroups(p.order,p.rows,p.cols).length;this.status=after<before?'拼接成功，块组已合并':'拖动图片块；已拼接部分会整体移动';if(after<before)this.audio.correct();this.analytics.track('puzzle_group_move',{level_id:this.game.level.level_id,source,target,group_size:groupForCell(next,p.rows,p.cols,target).length,moves:p.moves,groups_remaining:after});this.saveAttempt();if(isSolved(p.order)){this.audio.correct();this.finishAfterFeedback()}return true }
  markFound(difference) {
    ...
    if (this.preferences.data.vibration && navigator.vibrate) navigator.vibrate(18)
  ...
  renderLogin() {
    ...
    if (this.logo) r.image(this.logo, { x: 410, y: 42 + top, w: 260, h: 220 }, 'contain')
    r.text(this.i18n.t('app'), 540, 300 + top, 58, COLORS.gold, 'center', 'bold')
    r.text(this.i18n.t('tagline'), 540, 365 + top, 25, COLORS.muted, 'center')
    const adviceTop = Math.max(1160 + top, h * .70)
    const adviceBottom = h - (bottom + 40)
    const adviceAvailable = Math.max(200, adviceBottom - adviceTop)
    const adviceCenter = (adviceTop + adviceBottom) / 2
    const titleSize = Math.min(36, Math.round(adviceAvailable * 0.12))
    const lineSize = Math.min(28, Math.round(adviceAvailable * 0.095))
  ...
  onWheel(event) {
    ...
    if (!image) return
    event.preventDefault()
    const previous = this.game.view.zoom
    const next = clamp(previous * Math.exp(-event.deltaY * .0015), 1, 4)
    if (Math.abs(next - previous) < .001) return
    this.game.view.zoom = next
    this.clampGameView()
    this.saveAttempt()
  }
