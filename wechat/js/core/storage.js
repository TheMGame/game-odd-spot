...
class ProgressStore {
  ...
  getOrCreate(levelId, version) {
    ...
    const created = { attempt_id: uuid(), start_idempotency_key: uuid(), level_version: Number(version), state: 'in_progress', ever_completed: Boolean(existing && (existing.ever_completed || ['local_completed', 'sync_queued', 'synced', 'completed'].includes(existing.state))), found: [], hints_used: 0, wrong_taps: 0, elapsed_ms: 0, score: 0, points: 0, best_score: 0, zoom: 1, view_offset_x: 0, view_offset_y: 0 }
