...
function createStoryState(story, saved) {
  ...
  return { node_id: nodeId, visited: Array.isArray(previous.visited) ? previous.visited.slice(0, 300) : [], variables: Object.assign({}, previous.variables || {}), evidence: Array.isArray(previous.evidence) ? previous.evidence.slice() : [], hotspots: Object.assign({}, previous.hotspots || {}), sequences: Object.assign({}, previous.sequences || {}), puzzles: Object.assign({}, previous.puzzles || {}), completed: Boolean(previous.completed), ending_id: previous.ending_id || '' }
}
...
class StoryRuntime {
  ...
  enter(id, remember = true) { if (!this.story.nodes[id]) return false; if (remember && !this.state.visited.includes(this.state.node_id)) this.state.visited.push(this.state.node_id); this.state.node_id = id; const node = this.node(); if (node.set) Object.assign(this.state.variables, node.set); if (node.evidence_id && !this.state.evidence.includes(node.evidence_id)) this.state.evidence.push(node.evidence_id); if (node.type === 'ending') { this.state.completed = true; this.state.ending_id = node.ending_id || id } return true }
  ...
  findHotspot(id) { const node = this.node(), spot = (node.hotspots || []).find((item) => String(item.id) === String(id)); if (!spot) return false; const found = this.state.hotspots[node.id || this.state.node_id] || []; if (!found.includes(spot.id)) found.push(spot.id); this.state.hotspots[node.id || this.state.node_id] = found; if (spot.evidence_id && !this.state.evidence.includes(spot.evidence_id)) this.state.evidence.push(spot.evidence_id); const required = Number(node.required || (node.hotspots || []).length); if (found.length >= required && node.next) this.enter(node.next); return true }
  selectSequence(id) { const node = this.node(), valid = (node.items || []).some((item) => String(item.id) === String(id)); if (!valid) return false; const selected = this.state.sequences[this.state.node_id] || []; if (selected.includes(id)) return false; selected.push(id); this.state.sequences[this.state.node_id] = selected; if (selected.length === node.correct_order.length) { if (selected.map(String).join('|') === node.correct_order.map(String).join('|')) { if (node.next) this.enter(node.next) } else this.state.sequences[this.state.node_id] = [] } return true }
